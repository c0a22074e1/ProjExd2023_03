import random
import sys
import time

import pygame as pg


WIDTH = 1600  # ゲームウィンドウの幅
HEIGHT = 900  # ゲームウィンドウの高さ

idx = 0  # オリジナル課題
BLACK = (0, 0, 0)  # オリジナル課題
score = 0  #　課題3


def draw_text(scrn, txt, x, y, siz, col): # オリジナル課題
    fnt = pg.font.Font(None, siz)
    sur = fnt.render(txt, True, col)
    x = x - sur.get_width()/2
    y = y - sur.get_height()/2
    scrn.blit(sur, [x, y])


def check_bound(area: pg.Rect, obj: pg.Rect) -> tuple[bool, bool]:
    """
    オブジェクトが画面内か画面外かを判定し，真理値タプルを返す
    引数1 area：画面SurfaceのRect
    引数2 obj：オブジェクト（爆弾，こうかとん）SurfaceのRect
    戻り値：横方向，縦方向のはみ出し判定結果（画面内：True／画面外：False）
    """
    yoko, tate = True, True
    if obj.left < area.left or area.right < obj.right:  # 横方向のはみ出し判定
        yoko = False
    if obj.top < area.top or area.bottom < obj.bottom:  # 縦方向のはみ出し判定
        tate = False
    return yoko, tate


class character:  # 練習問題0

    def __init__(self, _rct):
        self._rct = _rct

    def get_rct(self):
        return self._rct


class Bird(character):
    """
    ゲームキャラクター（こうかとん）に関するクラス
    """
    _delta = {  # 押下キーと移動量の辞書
        pg.K_UP: (0, -1),
        pg.K_DOWN: (0, +1),
        pg.K_LEFT: (-1, 0),
        pg.K_RIGHT: (+1, 0),
    }

    def __init__(self, num: int, xy: tuple[int, int]):
        """
        こうかとん画像Surfaceを生成する
        引数1 num：こうかとん画像ファイル名の番号
        引数2 xy：こうかとん画像の位置座標タプル
        """
        self._img = pg.transform.flip(  # 左右反転
            pg.transform.rotozoom(  # 2倍に拡大
                pg.image.load(f"ex03/fig/{num}.png"), 
                0, 
                2.0), 
            True, 
            False
        )
        self._rct = self._img.get_rect()
        self._rct.center = xy

    def change_img(self, num: int, screen: pg.Surface):
        """
        こうかとん画像を切り替え，画面に転送する
        引数1 num：こうかとん画像ファイル名の番号
        引数2 screen：画面Surface
        """
        self._img = pg.transform.rotozoom(pg.image.load(f"ex03/fig/{num}.png"), 0, 2.0)
        screen.blit(self._img, self._rct)

    def update(self, key_lst: list[bool], screen: pg.Surface):
        """
        押下キーに応じてこうかとんを移動させる
        引数1 key_lst：押下キーの真理値リスト
        引数2 screen：画面Surface
        """
        for k, mv in __class__._delta.items():
            if key_lst[k]:
                self._rct.move_ip(mv)
        if check_bound(screen.get_rect(), self._rct) != (True, True):
            for k, mv in __class__._delta.items():
                if key_lst[k]:
                    self._rct.move_ip(-mv[0], -mv[1])
        screen.blit(self._img, self._rct)


        for k, mv in __class__._delta.items():
            if key_lst[k]:
                self._rct.move_ip(mv)
        if check_bound(screen.get_rect(), self._rct) != (True, True):
            for k, mv in __class__._delta.items():
                if key_lst[k]:
                    self._rct.move_ip(-mv[0], -mv[1])
        screen.blit(self._img, self._rct)


    def check_collide():  # 練習問題0
        check_bound(character, character)


class Bomb(character):
    """
    爆弾に関するクラス
    """
    def __init__(self, color: tuple[int, int, int], rad: int):
        """
        引数に基づき爆弾円Surfaceを生成する
        引数1 color：爆弾円の色タプル
        引数2 rad：爆弾円の半径
        """
        self._img = pg.Surface((2*rad, 2*rad))
        pg.draw.circle(self._img, color, (rad, rad), rad)
        self._img.set_colorkey((0, 0, 0))
        self._rct = self._img.get_rect()
        self._rct.center = random.randint(0, WIDTH), random.randint(0, HEIGHT)
        self._vx, self._vy = +1, +1

    def update(self, screen: pg.Surface):
        """
        爆弾を速度ベクトルself._vx, self._vyに基づき移動させる
        引数 screen：画面Surface
        """
        yoko, tate = check_bound(screen.get_rect(), self._rct)
        if not yoko:
            self._vx *= -1
        if not tate:
            self._vy *= -1
        self._rct.move_ip(self._vx, self._vy)
        screen.blit(self._img, self._rct)

    def check_collide():  # 練習問題0
        check_bound(character, character)


class Beam(character):  # 練習問題1
    def __init__(self, bird:Bird):

        self.beam = pg.image.load("ex03/beam.png")

        self._rct = self.beam.get_rect()
        self._rct.right = bird.get_rct().right
        self._rct.centery = bird.get_rct().centery


    def update(self):
        self._rct.x += 3

"""
class Explosion:  # 課題1
    eff = []
    def __init__(self, xy: tuple[int, int]):
        self.eff1 = pg.transform.flip(
            pg.image.load("ex03/explosion.gif"), 
            True, 
            True
        )  # 反転画像
        self.eff2 =pg.image.load("ex03/explosion.gif")  # 元画像
    
        eff = [self.eff1, self.eff2]

        self._rct = self.eff.get_rect()
        self._rct.center = xy

        time._life(1)

    def update():
        for i in range(time._life):
            eff_switch_interval = 100
"""

def main():
    global idx, score

    pg.display.set_caption("たたかえ！こうかとん")
    screen = pg.display.set_mode((WIDTH, HEIGHT))
    clock = pg.time.Clock()
    bg_img = pg.image.load("ex03/fig/pg_bg.jpg")

    bird = Bird(3, (900, 400))
    bomb = Bomb((255, 0, 0), 10)

    tmr = 0

    beams = []  # 練習問題1

    while True:
        for event in pg.event.get():
            if event.type == pg.QUIT:
                return

            elif event.type == pg.KEYDOWN:  # 練習問題1
                if event.key == pg.K_SPACE:
                    beam = Beam(bird)
                    beams.append(beam)

        tmr += 1
        screen.blit(bg_img, [0, 0])

        key = pg.key.get_pressed()  # オリジナル課題

        if idx == 0:  # オリジナル課題
            draw_text(screen, "SPACE INVADERS", 900, 300, 50, BLACK)
            draw_text(screen, "Press [1] to start!", 900, 600, 50, BLACK)
        if key[pg.K_1] == 1:
            pg.display.update()
            time.sleep(0.01)
            idx = 1
            score = 0  # 課題3
            bird = Bird(3, (900, 400))
            bomb = Bomb((255, 0, 0), 10)
        
        if bird._rct.colliderect(bomb._rct):
            # ゲームオーバー時に，こうかとん画像を切り替え，1秒間表示させる
            bird.change_img(8, screen)
            pg.display.update()
            time.sleep(1)
            return

        for beam in beams:  # 練習問題1
            if bomb._rct.colliderect(beam._rct):
                
                score += 1  # 課題3

                beams.remove(beam)
                # bomb = False
                # screen.blit()  # 課題1
                bird.change_img(9, screen)  # 練習問題2
                pg.display.update()  # 練習問題2
                time.sleep(1)  # 練習問題2
                bird.change_img(3, screen)
                pg.display.update()

        draw_text(screen, "SCORE "+str(score), 200, 30, 50, BLACK)  # 課題3

        key_lst = pg.key.get_pressed()
        bird.update(key_lst, screen)
        bomb.update(screen)

        for beam in beams:  # 練習問題1
            beam.update()
            screen.blit(beam.beam, beam._rct)

        pg.display.update()
        clock.tick(1000)


if __name__ == "__main__":
    pg.init()
    main()
    pg.quit()
    sys.exit()
